...
class GPUInference:
    ...
    def predict(self, full_image: np.ndarray) -> np.ndarray:
        """ Executes the model on the given image on a GPU.
        ...
        try:
            ...
            final_output = output_canvas[self.margin : self.margin+h, self.margin : self.margin+w].copy()
            ...
            # CLEANUP
            if img_padded is not None: del img_padded
            if output_canvas is not None: del output_canvas
            
            if tiles is not None: del tiles
            if batch_input is not None: del batch_input
            if batch_output is not None: del batch_output
    ...
    def release_resources(self):
        """ Forcefully releases the ONNX Session and frees GPU memory (VRAM)."""
        ...
        # Delete the wrapper if it exists
        if hasattr(self, 'model_wrapper'):
            del self.model_wrapper

        # Clear internal config variables
        if hasattr(self, 'input_name'): del self.input_name
        if hasattr(self, 'output_name'): del self.output_name
